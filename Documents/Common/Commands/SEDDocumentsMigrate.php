...
class SEDDocumentsMigrate extends Command
{
	protected $signature = 'sed-documents:migrate
                            {Module? : Указать название модуля (не обязательный)}
                            {--reset : Переустановить таблицы}
                            {--remove : Удалить таблицы включая память миграций}
                            {--back= : Откатить миграцию на n шагов}';

	protected $description = 'Миграции модуля управления документами СЭД';
	...
	{
		if ($this->hasArgument('Module') && !empty($this->argument('Module'))) {
			$table_name = strtolower('l_sed_' . $this->argument('Module') . '_migration');
		} else {
			$table_name = strtolower('l_sed_documents_migration');
		}
	...
	{
		if ($this->hasArgument('Module') && !empty($this->argument('Module'))) {
			$path = '/Modules/SED/Documents/' . $this->argument('Module') . '/Migrations';
		} else {
			$path = '/Modules/SED/Documents/Common/Migrations';
		}
