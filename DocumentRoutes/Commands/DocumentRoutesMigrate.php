...
class DocumentRoutesMigrate extends Command
{
	protected $signature = 'sed-document-routes:migrate
                            {--reset : Переустановить таблицы}
                            {--remove : Удалить таблицы включая память миграций}
                            {--back= : Откатить миграцию на n шагов}';

	protected $description = 'Миграции модуля маршрутов документов СЭД';
	...
	{
		return 'l_route_migration';
	}

	protected function getMigrationsPath(): string
	{
		return '/Modules/SED/DocumentRoutes/Migrations';
	}
